<?php require "layout/header.php" ?>
<!-- Main Content -->
<main class="roomList">
    <section class="search-container">
        <h5><i class="fas fa-search"></i> TÌM KIẾM & LỌC</h5>
        <form>
            <div class="row">
                <div class="form-group col-sm-12">
                    <label for="branch">Chọn chi nhánh:</label>
                    <select class="form-control" id="branch">
                        <option value="">-- Chọn chi nhánh --</option>
                    </select>
                </div>
                <div class="form-group col-xs-12 col-sm-6">
                    <label for="checkin">Ngày nhận:</label>
                    <input type="text" class="form-control" id="checkin" placeholder="Thứ 2, 28. Thg 5, 2025" />
                </div>
                <div class="form-group col-xs-12 col-sm-6">
                    <label for="checkout">Trả phòng:</label>
                    <input type="text" class="form-control" id="checkout" placeholder="Thứ 2, 28. Thg 5, 2025" />
                </div>
                <div class="form-group col-xs-12 col-sm-6">
                    <label for="roomtype">Loại phòng:</label>
                    <select class="form-control" id="roomtype">
                        <option value="VIP ROOM">VIP ROOM</option>
                        <option value="VIP ROOM">VIP 1 ROOM</option>
                        <option value="VIP ROOM">VIP 2 ROOM</option>
                        <option value="VIP ROOM">VIP 3 ROOM</option>
                    </select>
                </div>
                <div class="form-group col-xs-12 col-sm-6">
                    <label for="price">Giá:</label>
                    <select class="form-control" id="price">
                        <option value="1.700.000-2.500.000đ">
                            1.700.000-2.500.000đ
                        </option>
                        <option value="2.600.000-3.500.000đ">
                            2.600.000-3.500.000đ
                        </option>
                    </select>
                </div>
                <div class="form-group col-lg-12">
                    <button type="submit" class="search-btn">Tìm Phòng</button>
                </div>
            </div>
        </form>
    </section>

    <section class="room-section">
        <h5><i class="fas fa-star"></i> Phòng nổi bật</h5>
        <div class="row">
            <div class="col-xs-12 col-md-4">
                <div class="room-card">
                    <img src="../images/hotel.png" alt="Room 1" />
                    <div class="card-body">
                        <h6 class="card-title">VIP 1 Room</h6>
                        <p class="rating">★★★★★ (8.7/10)(255)</p>
                        <p class="coupon">2,200,000 VND</p>
                        <p class="price">1,800,000 VND</p>
                        <a href="#"><button class="btn">Book Now</button></a>
                        <a href="#"><button class="btn">Detail</button></a>
                    </div>
                </div>
            </div>
            <div class="col-xs-12 col-md-4">
                <div class="room-card">
                    <img src="../images/hotel.png" alt="Room 2" />
                    <div class="card-body">
                        <h6 class="card-title">VIP 22 Room</h6>
                        <p class="rating">★★★★☆ (8.7/10)(255)</p>
                        <p class="coupon">2,300,000 VND</p>
                        <p class="price">1,900,000 VND</p>
                        <a href="#"><button class="btn">Book Now</button></a>
                        <a href="#"><button class="btn">Detail</button></a>
                    </div>
                </div>
            </div>
            <div class="col-xs-12 col-md-4">
                <div class="room-card">
                    <img src="../images/hotel.png" alt="Room 3" />
                    <div class="card-body">
                        <h6 class="card-title">VIP 3 Room</h6>
                        <p class="rating">★★★★☆ (8.7/10)(255)</p>
                        <p class="coupon">2,300,000 VND</p>
                        <p class="price">2,000,000 VND</p>
                        <a href="#"><button class="btn">Book Now</button></a>
                        <a href="#"><button class="btn">Detail</button></a>
                    </div>
                </div>
            </div>
        </div>
    </section>

    <section class="room-section">
        <h5><i class="fas fa-star"></i> Phòng ưu đãi</h5>
        <div class="row">
            <div class="col-xs-12 col-md-4">
                <div class="room-card">
                    <img src="../images/hotel.png" alt="Room 4" />
                    <div class="card-body">
                        <h6 class="card-title">Tiêu chuẩn 1 Room</h6>
                        <p class="rating">★★★★☆ (8.7/10)(255)</p>
                        <p class="coupon">2,300,000 VND</p>
                        <p class="price">1,500,000 VND</p>
                        <a href="#"><button class="btn">Book Now</button></a>
                        <a href="#"><button class="btn">Detail</button></a>
                    </div>
                </div>
            </div>
            <div class="col-xs-12 col-md-4">
                <div class="room-card">
                    <img src="../images/hotel.png" alt="Room 5" />
                    <div class="card-body">
                        <h6 class="card-title">Tiêu chuẩn 22 Room</h6>
                        <p class="rating">★★★★☆ (8.7/10)(255)</p>
                        <p class="coupon">2,300,000 VND</p>
                        <p class="price">1,600,000 VND</p>
                        <a href="#"><button class="btn">Book Now</button></a>
                        <a href="#"><button class="btn">Detail</button></a>
                    </div>
                </div>
            </div>
            <div class="col-xs-12 col-md-4">
                <div class="room-card">
                    <img src="../images/hotel.png" alt="Room 6" />
                    <div class="card-body">
                        <h6 class="card-title">Tiêu chuẩn 3 Room</h6>
                        <p class="rating">★★★★☆ (8.7/10)(255)</p>
                        <p class="coupon">2,300,000 VND</p>
                        <p class="price">1,700,000 VND</p>
                        <a href="#"><button class="btn">Book Now</button></a>
                        <a href="#"><button class="btn">Detail</button></a>
                    </div>
                </div>
            </div>
        </div>
    </section>
</main>
<?php require "layout/footer.php" ?>